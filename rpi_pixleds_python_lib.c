...
#define SMI_TIMING       2, 6, 13, 6   // 50 ns cycle time
#endif

/*
Analysis:
Ref: https://iosoft.blog/2020/07/16/raspberry-pi-smi/
step, setup, hold, strobe
step = clock divider (ns), even!
strobe = setup * 2 = hold * 2 (nice to have for symmetrical waveform)

10, 10, 20, 10 -> 400ns (2,5 MS/s)
...
T1H = 950ns
T1L = 600ns
T0H + T0L = T1H + T1L = 1550ns
Cycle time = 50ns (20 MS/s) -> SMI_TIMING = 2, 6, 13, 6
BIT_NPULSES = 1550ns/50ns = 31
First 6 bits always 1
Middle 13 bits 0 or 1 depending on logic H/L
Last 12 bits always 0
*/
...
#define BIT_NPULSES     31   // Number of O/P pulses per LED bit
...
// Logic 1 is 0.7us high, 0.6 us low, logic 0 is 0.35us high, 0.95us low
void rgb_txdata(int *rgbs, TXDATA_T *txd)
{
    int i, n, msk;
    ...
    for (n=0; n<LED_NBITS; n++)
    {
        ...
        // First 6 bytes/words are high pulses on all lines
        // TODO use memset (how to do 8/16 bit portable?)
        for (int j=0; j<6; j++)
        {
            txd[j] = (TXDATA_T)0xffff;
        }
        // Middle 13 bytes/words are from data
        // Last 13 bytes/words are low pulses
        for (int j=6; j<BIT_NPULSES; j++)
        {
            txd[j] = 0;
        }
