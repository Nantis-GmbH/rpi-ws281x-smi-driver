...
T1H = 700ns
T1L = 600ns
T0H + T0L = T1H + T1L = 1300ns
Cycle time = 50ns (20 MS/s) -> SMI_TIMING = 2, 6, 13, 6
BIT_NPULSES = 1300ns/50ns = 26
First 7 bits always 1
Middle 7 bits 0 or 1 depending on logic H/L
...
#define BIT_NPULSES     26   // Number of O/P pulses per LED bit
...
void rgb_txdata(int *rgbs, TXDATA_T *txd)
{
    ...
    for (n=0; n<LED_NBITS; n++)
    {
        ...
        // First 7 bytes/words are high pulses on all lines
        // TODO use memset (how to do 8/16 bit portable?)
        for (int j=0; j<7; j++)
        {
            txd[j] = (TXDATA_T)0xffff;
        }
        // Middle 7 bytes/words are from data
        // Last 12 bytes/words are low pulses
        for (int j=7; j<26; j++)
        {
            txd[j] = 0;
        }
